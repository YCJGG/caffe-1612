...
void ConfusionMatrix::printF1Score(const char *header) const {
  ...
  LOG(INFO) << ROW_BEGIN;
  for (size_t i = 0; i < _matrix.size(); i++) {
    LOG(INFO) << 2.0 * _matrix[i][i] / (rowSum(i) + colSum(i));
  }
  LOG(INFO) << ROW_END;
}
...
double ConfusionMatrix::fg_accuracy() const {
  double total_sum = totalSum() - rowSum(0);
  double diag_sum  = diagSum() - _matrix[0][0];
...
double ConfusionMatrix::avgPrecision() const {
  ...
  return totalPrecision /= (double)count;//_matrix.size();
}
...
/*
* @ param from & to: calculate F1 score with label [from, to]
*/
double ConfusionMatrix::classF1(int from, int to) const {
