...
void MultiStageMeanfieldLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  /*
   *  Additional part: no need to read .par files to initialize filter weights
   *  Add on: 2016-03-28, @ Wei Zhen
   */
  double spatial_filter_weight = meanfield_param.spatial_filter_weight();
  double bilateral_filter_weight = meanfield_param.bilateral_filter_weight();
