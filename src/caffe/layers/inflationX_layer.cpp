/*
# Inflation X plan. Factor from side branch network.
# As factor is dynamic, batch size is 1 during training.
#
# Author: Wei Zhen @ IIE, CAS
# Create on: 2016-11-26
# Last modify: 2017-03-04
#
*/
...
void InflationXLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

    // init inflation factor
    //const Dtype* factor_ = bottom[1]->cpu_data();
    // check and correct factor's value range
    //CHECK_LE(*factor_, this->MAX_FACTOR);
    //CHECK_GE(*factor_, this->MIN_FACTOR);
    //if (*factor_ > this->MAX_FACTOR)	*factor_ = this->MAX_FACTOR;
    //else if (*factor_ < this->MIN_FACTOR)	*factor_ = this->MIN_FACTOR;
    ...
    if (inflation_param.use_bg_mask() == true)
    {
...
template <typename Dtype>
void InflationXLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    // blob shape check
...
template <typename Dtype>
void InflationXLayer<Dtype>::inflate_forward(const Dtype *bottom_data, const int bottom_height, const int bottom_width, 
...
template <typename Dtype>
void InflationXLayer<Dtype>::Forward_cpu(
...
template <typename Dtype>
void InflationXLayer<Dtype>::inflate_backward(Dtype *bottom_diff, const int bottom_height, const int bottom_width, 
...
template <typename Dtype>
void InflationXLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(InflationXLayer);
#endif
INSTANTIATE_CLASS(InflationXLayer);
REGISTER_LAYER_CLASS(InflationX);
}  // namespace caffe
