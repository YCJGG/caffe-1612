...
__global__ void nearestForwardGPU(const int nthreads,
	...
	const int output_height, const int output_width, const float resize_factor,
	const Dtype* bottom_data, Dtype* top_data) {

	CUDA_KERNEL_LOOP(index, nthreads) {
		// resume channel idx and pixel idx
		int rw = index % output_width;
		int rh = (index / output_width) % output_height;
		int c = (index / output_width / output_height) % input_channels;
		int n = index / output_width / output_height / input_channels;
		// indexing and sampling
		int h = int(rh / resize_factor);
		int w = int(rw / resize_factor);
		h = min(h, input_height);
		w = min(w, input_width);
		const Dtype* bottom_data_channel = bottom_data + (n * input_channels + c) * input_height * input_width;
		top_data[index] = bottom_data_channel[h * input_width + w];
	}
}
...
__global__  void bilinearForwardGPU(const int nthreads,
	...
	CUDA_KERNEL_LOOP(index, nthreads) {
		...
		int rh = (index / output_width) % output_height;
		int c = (index / output_width / output_height) % input_channels;
		int num = index / output_width / output_height / input_channels;
		// indexing and sampling
		float h = rh / resize_factor;
		float w = rw / resize_factor;
		//h = min(h, float(input_size));
		//w = min(w, float(input_size));
		const Dtype* bottom_data_channel = bottom_data + (num * input_channels + c) * input_height * input_width;
		top_data[index] = 0;		// DO NOT forget to reset before accumulation
		for (int n = MAX(floor(h-1) + 1, 0); n < MIN(h + 1, input_height); n++) {
		    for (int m = MAX(floor(w-1) + 1, 0); m < MIN(w + 1, input_width); m++) {
			top_data[index] += bottom_data_channel[n * input_width + m] * (1 - abs(w - m)) * (1 - abs(h - n));
		    }
		}
	}
}
  ...
  switch (this->layer_param_.resize_param().intepolation_type()) {
	case ResizeParameter_InterpolationType_NEAREST:
		...
		nearestForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>
			(count, bottom[0]->channels(), this->input_height_, this->input_width_,
			 this->output_height_, this->output_width_,
...
__global__ void nearestBackwardGPU(const int nthreads,
	...
	CUDA_KERNEL_LOOP(index, nthreads) {
		...
		rh = min(rh, output_height - 1);
		rw = min(rw, output_width - 1);
		top_diff += (n*input_channels + c) * output_height * output_width;
		bottom_diff[index] = top_diff[rh * output_width + rw];
	}
}
...
__global__ void bilinearBackwardGPU(const int nthreads,
	...
	CUDA_KERNEL_LOOP(index, nthreads) {
		...
		hstart = max(hstart, 0.);
		wstart = max(wstart, 0.);
		hend = min(hend, float(output_height));
		wend = min(wend, float(output_width));
		const Dtype* top_diff_channel = top_diff + (n*input_channels + c) * output_height * output_width;
		bottom_diff[index] = 0;
		for (int rh = hstart; rh < hend; ++rh) {
		    for (int rw = wstart; rw < wend; ++rw) {
			bottom_diff[index] += top_diff_channel[rh * output_width + rw]
  ...
  switch (this->layer_param_.resize_param().intepolation_type()) {
	case ResizeParameter_InterpolationType_NEAREST:
		...
		nearestBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(bottom_count), CAFFE_CUDA_NUM_THREADS>>>
			(bottom_count, bottom[0]->channels(), this->input_height_, this->input_width_,
			 this->output_height_, this->output_width_,
