...
// for image_seg_data layer
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndSegAndAddChannel(const std::vector<cv::Mat>& cv_img_seg,
  Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_label_blob, Blob<Dtype>* transformed_add_ch_blob,
  const int ignore_label) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain image, seg and additional channel.";

  // Check blob sizes
  const int img_channels = cv_img_seg[0].channels();
  int img_height   = cv_img_seg[0].rows;		  // height and width may change due to pad for cropping
  int img_width    = cv_img_seg[0].cols;
  ...
  const int add_ch_channels = cv_img_seg[2].channels();
  int add_ch_height   = cv_img_seg[2].rows;
  int add_ch_width    = cv_img_seg[2].cols;
  ...
  const int add_top_channels = transformed_add_ch_blob->channels();
  const int add_top_height   = transformed_add_ch_blob->height();
  const int add_top_width    = transformed_add_ch_blob->width();
  ...
  CHECK_EQ(add_ch_channels, 1);
  CHECK_EQ(add_top_channels, 1);
  CHECK_EQ(data_height, add_top_height);
  CHECK_EQ(data_width, add_top_width);

  CHECK(cv_img_seg[0].depth() == CV_8U) << "Image data type must be unsigned byte";
  CHECK(cv_img_seg[1].depth() == CV_8U) << "Seg data type must be unsigned byte";
  CHECK(cv_img_seg[2].depth() == CV_8U) << "Additional channel data type must be unsigned byte";
  ...
  // Fetch mean, but substract mean later
  Dtype* mean = NULL;
  if (has_mean_file) {
    CHECK_EQ(img_channels, data_mean_.channels());
    CHECK_EQ(img_height, data_mean_.height());
  ...
  cv::Mat cv_cropped_img = cv_img_seg[0];  
  cv::Mat cv_cropped_seg = cv_img_seg[1];
  cv::Mat cv_cropped_add_ch = cv_img_seg[2];
  
  cv_cropped_img.convertTo(cv_cropped_img, CV_64F);	// transform to double, since we will pad mean pixel values
  ...
  if (pad_height > 0 || pad_width > 0) {
    ...
    cv::copyMakeBorder(cv_cropped_seg, cv_cropped_seg, 0, pad_height, 
          0, pad_width, cv::BORDER_CONSTANT, 
		       cv::Scalar(ignore_label));
    cv::copyMakeBorder(cv_cropped_add_ch, cv_cropped_add_ch, 0, pad_height, 
          0, pad_width, cv::BORDER_CONSTANT, 
		       cv::Scalar(0));
    // update height/width
    img_height   = cv_cropped_img.rows;
    ...
    add_ch_height = cv_cropped_add_ch.rows;
    add_ch_width = cv_cropped_add_ch.cols;
  }
  ...
  if (crop_size) {
    ...
    cv_cropped_img = cv_cropped_img(roi);
    cv_cropped_seg = cv_cropped_seg(roi);
    cv_cropped_add_ch = cv_cropped_add_ch(roi);
  } 
  
  CHECK(cv_cropped_img.data);
  CHECK(cv_cropped_seg.data);
  CHECK(cv_cropped_add_ch.data);

  Dtype* transformed_data  = transformed_data_blob->mutable_cpu_data();
  Dtype* transformed_label = transformed_label_blob->mutable_cpu_data();
  Dtype* transformed_add_ch = transformed_add_ch_blob->mutable_cpu_data();

  int top_index;
  const double* data_ptr;
  const uchar* label_ptr;
  const uchar* add_ch_ptr;

  // Iterate on img/seg/add_ch, substract mean and do mirroring
  for (int h = 0; h < data_height; ++h) {
    data_ptr = cv_cropped_img.ptr<double>(h);
    label_ptr = cv_cropped_seg.ptr<uchar>(h);
    add_ch_ptr = cv_cropped_add_ch.ptr<uchar>(h);
	...
	if (do_mirror) {
      ...
      // for seg label & additional channel
      if (do_mirror) {
	top_index = h * data_width + data_width - 1 - w;
      } else {
	top_index = h * data_width + w;
      ...
      Dtype pixel = static_cast<Dtype>(label_ptr[label_index++]);
      transformed_label[top_index] = pixel;
      pixel = static_cast<Dtype>(add_ch_ptr[add_ch_index++]);
      transformed_add_ch[top_index] = pixel;
    }
  }
}
#endif  // USE_OPENCV
